...
def indicDMI(periodN, periodV, currency, redline, con, current_time):
    ...
    if (periodN2 > 20):
        periodN2 = 20
    y = 0
    data = con.get_candles(currency, period=periodV, number=periodN2, end=current_time[1])
...
def indicRSI(periodN, periodV, currency, redline, con, current_time):
    result = {
        'redline': False,
        'type': None,
    }
    ...
    y = 0
    z = 0
    data = con.get_candles(currency, period='H1', number=500, end=current_time[1])
...
def resultbycurrency(con, currency, period, periodCount, current_time):

    periodN = periodCount
    data = con.get_candles(currency, period=period, number=periodN, end=current_time[1])
    ...
    result['MVA'] = redline
    redline = indicDMI(periodN, 'm15', currency, redline, con, current_time)
    result['DMI'] = redline
    if (redline == True):
        print("You can buy now on " + currency)
        result['tendencyValidation'] = True
        redline = False
    rsiOutput = indicRSI(periodN, 'm15', currency, redline, con, current_time)
    result.update(rsiOutput)
    return result 

#   To get an accurate reading on  the lines,
...
def get_index(status, currency):
    for i in range(0, len(status[0])):
        if currency == status[0][i]:
            return i

def close_status(f, status, current_time, currency):
    i = get_index(status, currency)
    if status[1][i] == '1':
        f.write("Close, " + currency + ", " + str(current_time[1]) + "\n")
        status[1][i] = '0'
    return status

def open_status(f, status, action, current_time, currency):
    i = get_index(status, currency)
    if status[1][i] != '1':
        f.write(action + ", " + currency + ", " + str(current_time[1]) + "\n")
        status[1][i] = '1'
        status[2][i] = action == "Buy"
    return status

def run(con , current_time, status):
    # currencies = ["GBP/USD"]
    f = open("historic", "a")
    ...
    for currency in currencies:
        cand = con.get_candles(currency, period=mainPeriod, number=104, columns=['bids'], end=current_time[1])
        tendency = resultbycurrency(con, currency, mainPeriod, tendencySpan, current_time)
    ...
    for currency in currencies:
        cand = con.get_candles(currency, period=confirmationPeriod, number=104, columns=['bids'], end=current_time[1])
        cand.columns = ['Open', 'Close', 'High', 'Low']
        args = addToCandles(currency, cand)
        line = analyse(currency, cand, args)
    ...
    for currency in currencies:
        cand = con.get_candles(currency, period=validationPeriod, number=104, columns=['bids'], end=current_time[1])
        cand.columns = ['Open', 'Close', 'High', 'Low']
        args = addToCandles(currency, cand)
        line = analyse(currency, cand, args)
    ...
    for index, row in full.iterrows():
        ...
        if row['choice'] != None:
            ...
            #             #con.close_trade(trade_id=orders[o]['tradeId'], amount=orders[o]['amountK'])  # JE DOIS PAS CLOSE MAIS FAIRE UNE ARROW SELL
            ...
            if found == False:
                ...
                #     #order = con.open_trade(symbol=row['Currency'], is_buy=isBuy, amount=money, time_in_force='GTC', order_type='AtMarket', is_in_pips=False, stop=close, trailing_step=10)  # JE DOIS PAS OPEN MAIS FAIRE UNE ARROW BUY
                #     tries -= 1
                #     time.sleep(1)
                print('Created order : ', order)
                if order == 0:
                    print("Just blunt creating ?")
                    status = open_status(f, status, row['choice'], current_time, row['Currency'])
                    #print("Order? : ", o)
                # if rt != 0:
                    # oid = rt['tradeId']
                    # print("Oid : ", oid)
        ...
        for index, row in full.iterrows():
            if row['Currency'] == idx:
                if row['chikouClose']:
                    ...
                    status = close_status(f, status, current_time, row['Currency'])
                    #con.close_trade(trade_id=o['tradeId'], amount=o['amountK']) # JE DOIS PAS CLOSE MAIS FAIRE UNE ARROW SELL
                if row['turningPoint'] == True:
                    i = get_index(status, row['Currency'])
                    if status[2][i] == '1':
                        currentTrade = "Buy"
                    else:
                        currentTrade = "Sell"
                    if row['choice'] != None:
                        if (currentTrade and row['choice'] == "Sell") or (currentTrade == False and row['choice'] == "Buy"):
                            ...
                            status = close_status(f, status, current_time, currency)
    f.close()
    return status